...
## Status catch-up (2026-08-14)

The constellation caught up with the suite era: a sixth cluster — **verifiable
evaluation** — seeds vac-protocol, agent-certlab, reference-fleet, evalmut and
vac-gate as first-class worlds (painters, scenes, curated detail), the stale
worlds (model-drift's measured noise floor, crashkit's evidence bundle,
harness-builder's flagship framing) were refreshed, and the replay-gauntlet
story (three refusals, three layers, one afternoon — all public commits) lives
in the vac-protocol world. Every number in the new copy was verified against
the issuer repos' committed records before it shipped.
